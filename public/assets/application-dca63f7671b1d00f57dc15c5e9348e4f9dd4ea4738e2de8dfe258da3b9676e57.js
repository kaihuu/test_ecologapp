...
(function() {
  this.Graph = function(xdata, ydata, colordata) {
    var data, layout, myPlot, trace1;
    ...
    myPlot.on('plotly_click', function(data) {
      var i, j, pts, ref;
      pts = '';
      for (i = j = 0, ref = data.points.length - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
        pts = 'x = ' + data.points[i].x + '\ny = ' + data.points[i].y.toPrecision(4) + '\n\n';
      }
      return alert('Closet point clicked:\n\n' + pts);
    });
  };

  this.Mapgenerate = function(divname, latdata, longdata) {
    var geojson, map;
    ...
    geojson.features.forEach(function(marker) {
      var el, i, j, ref;
      for (i = j = 0, ref = marker.geometry.coordinates.length - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
